...
//	@Select({ "SELECT COUNT(content) FROM", TABLE_NAME, "WHERE idTopic=#{topic_id}" })
...
//	List<Answer> findAnswerByUser_IdAndUsefulOrderByCreatedDateDesc(@Param("user_id") Long user_id,
//			@Param("useful") boolean useful);
//
//	@Select({ "SELECT", SELECT_FIELDS, "FROM", TABLE_NAME, "WHERE topic_id={topic_id}" })
